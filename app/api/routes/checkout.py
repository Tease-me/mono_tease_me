...
def _verify_signature(raw_body: bytes, signature: str | None) -> None:
    ...
    if not secret:
        log.warning(
            "PAYMENT_WEBHOOK_SECRET is not configured — skipping signature verification"
        )
        return
...
async def payment_webhook(
    ...
    if payload.event != "payment.completed":
        ...
        return {
            "ok": True,
            "skipped": True,
            "reason": f"Unhandled event: {payload.event}",
        }
    ...
    if not topup:
        log.error(
            "No PayPalTopUp record for checkout_id=%s — cannot process",
            payload.checkout_id,
        )
    ...
    if topup.credited:
        log.info(
            "Duplicate webhook for checkout_id=%s — already processed",
            payload.checkout_id,
        )
    ...
    try:
        ...
        await db.rollback()
        log.exception(
            "payment_webhook: failed to credit wallet for checkout_id=%s",
            payload.checkout_id,
        )
    ...
    for item in payload.notificationItems:
        ...
        topup = await db.scalar(
            select(PayPalTopUp).where(
                PayPalTopUp.order_id == notification.merchantReference
            )
        )
        if not topup:
            log.error(
