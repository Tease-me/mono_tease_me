...
def test_step_progress_requires_valid_internal_token(monkeypatch) -> None:
    ...
    response = client.post(
        ...
        json={"invite_code": "invite-123", "new_user_email": "user@example.com"},
    )
...
def test_step_progress_returns_current_progress_without_mutation(monkeypatch) -> None:
    ...
    async def _fake_lookup(db_arg, *, invite_code: str, new_user_email: str):
        assert db_arg is db
        captured["invite_code"] = invite_code
        captured["new_user_email"] = new_user_email
        return pre
...
def test_step_progress_returns_404_for_missing_progress_target(monkeypatch) -> None:
    ...
    async def _fake_lookup(_db_arg, *, invite_code: str, new_user_email: str):
        return None
    ...
    response = client.post(
        "/mjpromoter/pre-influencers/step-progress",
        json={"invite_code": "missing", "new_user_email": "user@example.com"},
        headers={"X-Internal-Token": "internal-secret"},
    )
...
def test_step_progress_rejects_blank_invite_code(monkeypatch) -> None:
    ...
    response = client.post(
        "/mjpromoter/pre-influencers/step-progress",
        json={"invite_code": "   ", "new_user_email": "user@example.com"},
        headers={"X-Internal-Token": "internal-secret"},
    )
