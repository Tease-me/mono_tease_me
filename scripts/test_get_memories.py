#!/usr/bin/env python3
"""
Test script for the /webhooks/memories endpoint.

Simulates the ElevenLabs `getMemory` webhook call to exercise the
`find_similar_memories` (and union-based `find_similar_memories_and_messages`)
code path.

Usage:
    # Against local dev server (default)
    python scripts/test_get_memories.py

    # Override base URL
    python scripts/test_get_memories.py --base-url https://your-deployed-api.com

    # Custom conversation & text
    python scripts/test_get_memories.py \
        --conversation-id "abc123" \
        --text "Tell me about last weekend"

    # Test with bogus token to verify auth rejection
    python scripts/test_get_memories.py --bad-token
"""
