...
async def resume_invited_pre_influencer(
    ...
    is_mj_register = is_mj_referral_pre_influencer(pre)
...
async def register_pre_influencer(
    ...
    if is_mj_register:
        ...
        if existing_pre is not None:
            ...
            if isinstance(existing_pre, PreInfluencer):
...
async def list_pre_influencer_audio(
    ...
    keys = await pre_influencer_storage.list_audio_keys_with_legacy_id(
        pre.username.strip() if pre.username else None,
        str(pre.id),
    )
    files = [
        {
