...
export const InfluencerRepo = () => ({
  ...
  getInfluencer: async (
    ...
    try {
      const response: InfluencerResponse =
        await influencerServices.getInfluencer(influencer_id);
      return {
        id: response.id,
  ...
  patchInfluencer: async (
    ...
    try {
      const response: InfluencerResponse =
        await influencerServices.patchInfluencer(
          influencer.id,
          influencer.name,
  ...
  createInfluencer: async (influencer: InfluencerDataModel) => {
    try {
      const response: InfluencerResponse =
        await influencerServices.createInfluencer(
          influencer.id,
  ...
  uploadCsv: async (file: File, save: boolean): Promise<void> => {
    try {
      await influencerServices.uploadCsv(file, save);
    } catch (e) {
      throw e;
    }
  },
  listKnowledgeFiles: async (
    influencer_id: string
  ): Promise<KnowledgeFileModel[]> => {
    try {
      const response: KnowledgeFile[] =
        await influencerServices.listKnowledgeFiles(influencer_id);
      return response.map((item) => ({
  ...
  uploadKnowledgeFile: async (
    ...
    try {
      const item = await influencerServices.uploadKnowledgeFile(
        influencer_id,
        file
      );
      return {
